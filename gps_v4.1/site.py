...
def setHandler(response):
    time = float(response.get_field("time"))*3600
    uuid = response.get_field("uuid")
    ...
    response.write(TemplateAPI.render('set.html', response, {}))
...
def submitMapDSettings(response):
    time = float(response.get_field("timelim"))*3600
    if time != float([line for line in open("options.txt")][3]):
        old=[line for line in open("options.txt")][:3]
    f=open("options.txt",'w')
    for line in old:
        f.write(line)
    f.write(time)
...
def getUUIDList(response):
    cur = con.cursor()
    cur.execute("SELECT DISTINCT UUID FROM Positions")
    u=filter(lambda x: type(x) is int, [i[0] for i in list(cur.fetchall())])
...
def getLineData():
    lim = int(time.time()-float([line for line in open("options.txt")][3]))
    print(lim)
    cur = con.cursor()    
    data=[]
    ...
    cur.execute("SELECT UUID, Lat, Lon, Time FROM Positions WHERE Time>? ORDER BY Time DESC ", (str(lim),))
    data.append([list(i) for i in cur.fetchall()])
